...
try {
    ...
    if ($method === 'POST') {
        ...
        $kode = trim($input['KodeSupplier'] ?? '');
        $nama = $input['NamaSupplier'] ?? '';
        $alamat = $input['Alamat1'] ?? '';
        $kota = $input['Kota1'] ?? '';
        $npwp = $input['NPWP'] ?? '';
        $telp = $input['Telp1'] ?? '';
        $cp = $input['ContactPerson'] ?? '';
        ...
        $stmt = $conn->prepare("INSERT INTO m_supplier (KodeSupplier, NamaSupplier, Alamat1, Kota1, NPWP, Telp1, ContactPerson) VALUES (?, ?, ?, ?, ?, ?, ?)");
