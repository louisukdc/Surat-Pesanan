...
/**
 * Generate nomor surat pesanan unik (PO/YYYY/MM/NNNN)
 */
...
function db_create_purchase_order($no_pesanan, $tgl_pesanan, $nama_vendor, $harga_vendor, $diskon_vendor, $total_setelah_diskon, $status, $dibuat_oleh, $items, $extra = array()) {
    $no_pesanan          = substr(db_escape($no_pesanan), 0, 25);
    ...
    // Cek apakah no_permintaan sudah ada (abaikan jika kosong)
    if ($no_permintaan !== '') {
        $cek_minta = mysqli_query($GLOBALS['db_conn'], "SELECT no_sp FROM spu_h WHERE no_permintaan = '$no_permintaan' LIMIT 1");
    ...
    // Cek apakah no_tawar sudah ada (abaikan jika kosong)
    if ($no_tawar !== '') {
        $cek_tawar = mysqli_query($GLOBALS['db_conn'], "SELECT no_sp FROM spu_h WHERE no_tawar = '$no_tawar' LIMIT 1");
        if ($cek_tawar && mysqli_num_rows($cek_tawar) > 0) {
            $row = mysqli_fetch_assoc($cek_tawar);
            $GLOBALS['last_db_error'] = "Nomor Penawaran '$no_tawar' sudah terpakai pada SP: " . $row['no_sp'] . ". Nomor Penawaran tidak boleh ganda!";
            return false;
        }
    }
    ...
    try {
        // Ambil dan kunci counter SP terbaru
        $tahun = (int)date('Y');
        
        // Insert jika belum ada tahun ini
        mysqli_query($GLOBALS['db_conn'], "INSERT IGNORE INTO sp_counter (tahun, nomor) VALUES ($tahun, 0)");
        
        // Kunci baris counter (FOR UPDATE)
        $res_counter = mysqli_query($GLOBALS['db_conn'], "SELECT nomor FROM sp_counter WHERE tahun = $tahun FOR UPDATE");
        $row_counter = mysqli_fetch_assoc($res_counter);
        $new_nomor = (int)$row_counter['nomor'] + 1;
